...
using YoutubeDownloader.Features.VideoSummary; // For VideoSummaryViewModel?
...
public partial class DashboardViewModel : ViewModelBase
{
    ...
    private bool CanShowDownloadHistory() => !IsBusy;

    [RelayCommand(CanExecute = nameof(CanShowDownloadHistory))]
    private async Task ShowDownloadHistoryAsync() =>
        await _dialogManager.ShowDialogAsync(_viewModelManager.CreateDownloadHistoryViewModel());

    [RelayCommand]
    private async Task ShowVideoSummaryAsync(DownloadViewModel download)
    {
        if (download?.FilePath is null || download.Video is null)
            return;

        await _dialogManager.ShowDialogAsync(
            _viewModelManager.CreateVideoSummaryViewModel(
                download.FilePath,
    ...
    private async void EnqueueDownload(DownloadViewModel download, int position = 0)
    {
        ...
        try
        {
            ...
            _downloadHistoryService.AddEntry(
                new DownloadHistoryEntry
                {
                    VideoId = download.Video!.Id,
                    Title = download.Video!.Title,
                    Author = download.Video!.Author.ChannelTitle,
                    DownloadedAt = DateTimeOffset.Now,
                    FilePath = download.FilePath!,
                    Format = downloadOption.Container.Name,
                }
            );
        }
        catch (Exception ex)
